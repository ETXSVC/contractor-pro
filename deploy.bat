...
ssh root@%VPS% ^
  "docker network create contractor-net 2>/dev/null; ^
   docker run -d ^
     --name contractor-db ^
     --network contractor-net ^
     --restart unless-stopped ^
     -e POSTGRES_USER=%DB_USER% ^
     -e POSTGRES_PASSWORD=%DB_PASS% ^
     -e POSTGRES_DB=%DB_NAME% ^
     -v contractor_pgdata:/var/lib/postgresql/data ^
...
ssh root@%VPS% ^
  "docker pull %IMAGE%:latest && ^
   docker stop contractor-pro 2>/dev/null; ^
   docker rm contractor-pro 2>/dev/null; ^
   docker run -d ^
     --name contractor-pro ^
     --network contractor-net ^
     --restart unless-stopped ^
